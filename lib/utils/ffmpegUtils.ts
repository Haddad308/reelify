...
function wrapTextForFFmpeg(
  ...
  if (typeof window !== 'undefined' && typeof document !== 'undefined') {
    try {
      ...
      console.warn('[wrapTextForFFmpeg] Failed to use canvas measureText, falling back to estimation:', err);
    }
  }
...
function buildSimpleCaptionFilter(
  ...
  if (captionEnd <= captionStart) {
    console.warn(`[buildSimpleCaptionFilter] Skipping caption ${caption.id} - outside trim range:`, {
  ...
  const padding = caption.style.padding || { top: 0, right: 0, bottom: 0, left: 0 };
  ...
  const wrappedText = wrapTextForFFmpeg(text, caption.style.fontSize, caption.style.fontFamily, maxTextWidth);
  const lines = wrappedText.split('\n');
  
  console.log(`[buildSimpleCaptionFilter] Caption text: ${lines.length} lines`);
  console.log(`[buildSimpleCaptionFilter] Lines:`, lines.map((line, i) => `${i+1}: "${line.substring(0, 30)}..."`));
  ...
  if (typeof window !== 'undefined' && typeof document !== 'undefined') {
    try {
      ...
      console.warn('[buildSimpleCaptionFilter] Failed to measure text width, using estimation:', err);
    }
  }
  ...
  if (hasArabic) {
    console.log(`[buildSimpleCaptionFilter] Caption contains Arabic text - will use font file and text shaping`);
  }
  ...
  if (typeof window !== 'undefined' && typeof document !== 'undefined') {
    try {
      ...
      if (ctx) {
        ...
        console.log(`[buildSimpleCaptionFilter] Measured widest line ${widestLineIndex + 1}: ${Math.round(widestLineWidth)}px`);
      }
    } catch (err) {
  ...
  const createLineFilters = (lineText: string, lineIndex: number, includeBox: boolean): string[] => {
  ...
  const createMixedLineFilters = (lineText: string, lineIndex: number, includeBox: boolean, referenceWidth?: number, videoWidth?: number): string[] => {
    ...
    const segmentData: Array<{text: string, width: number, isArabic: boolean}> = [];
    ...
    if (typeof window !== 'undefined' && typeof document !== 'undefined') {
      try {
        ...
        if (ctx) {
          ...
          if (measuredLineWidth > 0 && totalWidth > 0 && Math.abs(measuredLineWidth - totalWidth) > 1) {
    ...
    if (includeBox && caption.style.backgroundColor && caption.style.backgroundColor !== 'transparent') {
      ...
      if (bgColor) {
        ...
        console.log(`[buildSimpleCaptionFilter] Line ${lineIndex + 1}: Background box created for mixed content (width: ${Math.round(boxWidth)}px, x=${boxX}, videoWidth=${videoWidth})`);
      } else {
        console.warn(`[buildSimpleCaptionFilter] Line ${lineIndex + 1}: Failed to convert background color: ${caption.style.backgroundColor}`);
      }
    } else {
      console.log(`[buildSimpleCaptionFilter] Line ${lineIndex + 1}: No box (includeBox=${includeBox}, backgroundColor=${caption.style.backgroundColor})`);
    }
    ...
    if (typeof window !== 'undefined' && typeof document !== 'undefined') {
      try {
        ...
        if (textNode && textNode.nodeType === Node.TEXT_NODE) {
          ...
          segmentData.forEach((segment, segIndex) => {
            ...
            try {
              ...
              console.warn(`[createMixedLineFilters] Range API failed for segment ${segIndex}:`, err);
          ...
          visualOrder = segmentData.map((_, idx) => ({ index: idx, visualX: idx * 100 }));
        }
        ...
        console.log(`[createMixedLineFilters] Visual order determined:`, visualOrder.map(v => ({
          index: v.index,
          text: segmentData[v.index].text.substring(0, 20),
          visualX: Math.round(v.visualX)
        })));
      } catch (err) {
        console.warn('[createMixedLineFilters] Failed to determine visual order, using sequential:', err);
    ...
    if (useVisualOrder && visualOrder.length === segmentData.length) {
      ...
      const firstSegmentWidth = segmentWidths[firstSegmentIndex] || segmentData[firstSegmentIndex].text.length * caption.style.fontSize * 0.6;
    ...
    visualOrder.forEach((visualItem, visualIndex) => {
      ...
      const segmentWidth = segmentWidths[segIndex] || segment.text.length * caption.style.fontSize * 0.6;
      ...
      if (useVisualOrder && visualOrder.length === segmentData.length) {
        ...
        if (visualIndex === 0) {
          ...
          const prevSegmentWidth = segmentWidths[prevSegIndex] || segmentData[prevSegIndex].text.length * caption.style.fontSize * 0.6;
          currentX = baseX;
          // Calculate cumulative width up to this point in visual order
          for (let i = 0; i < visualIndex; i++) {
            const prevIdx = visualOrder[i].index;
            currentX += Math.round(segmentWidths[prevIdx] || segmentData[prevIdx].text.length * caption.style.fontSize * 0.6);
          }
        }
      }
      // Escape text for FFmpeg drawtext filter
      ...
      console.log(`[createMixedLineFilters] Positioning segment ${segIndex + 1} (visual pos ${visualIndex + 1}) at x=${currentX}, text="${segment.text.substring(0, 20)}..."`);
      ...
      if (segment.isArabic) {
        ...
        console.log(`[createMixedLineFilters] Segment ${segIndex + 1}: Arabic "${segment.text.substring(0, 20)}..." - using arabic.ttf WITHOUT text_shaping (WASM compatibility), fontsize=${segmentFontSize}`);
      } else {
        segmentOptions.push('fontfile=default.ttf');
        // NO text_shaping for English - it breaks rendering
        console.log(`[createMixedLineFilters] Segment ${segIndex + 1}: English "${segment.text.substring(0, 20)}..." - using default.ttf, fontsize=${segmentFontSize} (reduced from ${caption.style.fontSize} to match Arabic weight)`);
      }
    ...
    console.log(`[buildSimpleCaptionFilter] Line ${lineIndex + 1}: Mixed content split into ${segments.length} segments, total width: ${Math.round(totalWidth)}px`);
    return filters;
  };
  ...
  const createSingleLineFilter = (lineText: string, lineIndex: number, includeBox: boolean): string | string[] => {
    ...
    if (typeof window !== 'undefined' && typeof document !== 'undefined') {
      try {
        ...
        console.warn(`[buildSimpleCaptionFilter] Canvas measurement failed, using estimation:`, err);
        lineWidth = lineText.length * caption.style.fontSize * 0.5;
        if (includeBox) {
          lineX = Math.round(scaledX - (widestLineWidth / 2));
        } else {
    ...
    if (includeBox) {
      ...
      console.log(`[buildSimpleCaptionFilter] Line ${lineIndex + 1}: x=${lineX}, y=${lineY}, width=${Math.round(lineWidth)}, text="${lineText.substring(0, 20)}..."`);
    }
    ...
    if (lineHasArabic && !lineHasLatin) {
      ...
      console.log(`[buildSimpleCaptionFilter] Line ${lineIndex + 1}: Pure Arabic, arabic.ttf WITHOUT text_shaping (WASM compatibility)`);
      ...
      console.log(`[buildSimpleCaptionFilter] Line ${lineIndex + 1}: Pure English, default.ttf (Roboto)`);
      ...
      console.log(`[buildSimpleCaptionFilter] Line ${lineIndex + 1}: Mixed Arabic+English, arabic.ttf WITHOUT text_shaping - both scripts should render`);
    } else {
      // No script detected - default to arabic.ttf if caption has Arabic, otherwise default.ttf
      if (hasArabic) {
        lineOptions.push('fontfile=arabic.ttf');
        console.log(`[buildSimpleCaptionFilter] Line ${lineIndex + 1}: No script detected, using arabic.ttf (caption has Arabic)`);
      } else {
        lineOptions.push('fontfile=default.ttf');
        console.log(`[buildSimpleCaptionFilter] Line ${lineIndex + 1}: No script detected, using default.ttf`);
      }
    }
  ...
  widestLineFilters.forEach((filter, idx) => {
    console.log(`[buildSimpleCaptionFilter] Widest line ${widestLineIndex + 1} filter ${idx + 1}/${widestLineFilters.length} (with box): ${filter.substring(0, 100)}...`);
    filters.push(filter);
  });
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    lineFilters.forEach((filter, idx) => {
      console.log(`[buildSimpleCaptionFilter] Line ${i + 1} filter ${idx + 1}/${lineFilters.length}: ${filter.substring(0, 80)}...`);
      filters.push(filter);
    });
  }
  ...
  console.log(`[buildSimpleCaptionFilter] ✅ Built ${filters.length} filters (box on widest line ${widestLineIndex + 1}, drawn first)`);
  ...
  if (!finalFilter || finalFilter.length === 0) {
    console.error(`[buildSimpleCaptionFilter] ❌ ERROR: Empty filter generated for caption ${caption.id}`, {
  ...
  visibleCaptions.forEach((caption) => {
    ...
    if (caption.style.backgroundColor && caption.style.backgroundColor !== 'transparent') {
      ...
      if (caption.style.backgroundColor.startsWith('#')) {
        ...
        // Parse rgba color
        const match = caption.style.backgroundColor.match(/rgba?\s*\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*,\s*([\d.]+)\s*\)/);
        if (match) {
          const r = parseInt(match[1], 10).toString(16).padStart(2, '0');
          const g = parseInt(match[2], 10).toString(16).padStart(2, '0');
          const b = parseInt(match[3], 10).toString(16).padStart(2, '0');
          const alpha = Math.round((1 - parseFloat(match[4])) * 255).toString(16).padStart(2, '0'); // ASS uses inverse alpha
...
export function buildFFmpegCommand(
  ...
  if (shouldUseStreamCopy) {
    ...
    const needsReframing = settings.formatOptions?.reframing?.enabled && 
                          settings.formatOptions?.reframing?.mode !== 'none';
    const needsFormatChange = settings.formatOptions?.format !== 'portrait';
    ...
    if (!needsReframing && !needsFormatChange) {
      ...
      if (hasCaptions) {
        console.log(`[buildFFmpegCommand] Adding drawtext filters (stream copy fallback) for ${visibleCaptions.length} captions`);
        ...
        if (captionFilters && captionFilters.length > 0) {
          console.log(`[buildFFmpegCommand] Caption filters (stream copy):`, captionFilters.substring(0, 200) + '...');
          filterParts.push(captionFilters);
        } else {
          console.warn('[buildFFmpegCommand] No caption filters generated in stream copy fallback!');
        }
      } else {
    ...
    if (hasCaptions) {
      console.log(`[buildFFmpegCommand] Adding drawtext filters for ${visibleCaptions.length} captions:`, {
      ...
      const captionFilters = buildCaptionFilters(visibleCaptions, startTime, width, height);
      if (captionFilters && captionFilters.length > 0) {
        console.log(`[buildFFmpegCommand] Caption filters generated:`, captionFilters.substring(0, 200) + '...');
        filterParts.push(captionFilters);
      } else {
    ...
    if (filterParts.length > 0) {
      ...
      console.log(`[buildFFmpegCommand] Complete filter chain (${filterParts.length} filters):`, filterChain);
      console.log(`[buildFFmpegCommand] Filter breakdown:`, filterParts.map((f, i) => `${i + 1}. ${f.substring(0, 150)}...`));
