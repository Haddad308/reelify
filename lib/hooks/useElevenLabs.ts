...
export function useElevenLabs() {
  ...
  const transcribe = useCallback(
    async (audioUrl: string, language: 'ar' | 'en' = 'en'): Promise<ElevenLabsTranscriptionResult> => {
      setIsLoading(true);
      setError(null);
  ...
  const textToSpeech = useCallback(
    async (text: string, voiceId: string = 'default', language: 'ar' | 'en' = 'en'): Promise<string> => {
      setIsLoading(true);
      setError(null);
