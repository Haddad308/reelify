...
export class CanvasExportService {
  ...
  static async exportVideo(
    ...
    if (report.firstFrameBreakdown) {
      console.log('  First frame breakdown: seek', report.firstFrameBreakdown.seekMs.toFixed(0), 'ms, draw', report.firstFrameBreakdown.drawMs.toFixed(0), 'ms, toBlob', report.firstFrameBreakdown.toBlobMs.toFixed(0), 'ms');
    }
    console.log('  Frames:', report.totalFrames, '| FPS:', report.fps, '| Duration:', report.durationSec, 's');
    console.groupEnd();
