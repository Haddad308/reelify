  ...
  static async exportVideo(
    ...
    try {
      ...
      const executeAndCheck = async (args: string[]): Promise<Uint8Array | null> => {
        ...
        console.log('FFmpeg command (formatted):', args.map((arg, i) => {
          if (arg === '-vf' && args[i + 1]) {
        ...
        try {
          ...
          if (logs.length > 0) {
            ...
            const drawtextErrors = logs.filter(log => 
      ...
      console.log('Export captions check:', {
        ...
        captions: captions.map(c => ({ id: c.id, text: c.text.substring(0, 30), isVisible: c.isVisible, startTime: c.startTime, endTime: c.endTime }))
      });
      ...
      if (visibleCaptions.length > 0) {
        console.log('Loading font for subtitle rendering...', hasArabicCaptions ? '(Arabic/Mixed text detected)' : '(English text)');
        try {
            ...
            try {
              ...
              if (response.ok) {
                ...
                if (data.byteLength > 10000) {
                  arabicFontData = data;
                  console.log(`✓ Arabic font loaded from local: ${path} (${data.byteLength} bytes)`);
                  break;
                } else {
                  console.warn(`Arabic font file too small: ${data.byteLength} bytes (expected > 10000)`);
                }
              } else {
                console.warn(`Arabic font fetch failed for ${path}: ${response.status} ${response.statusText}`);
              }
            } catch (err) {
          ...
          if (!arabicFontData) {
            try {
              const arabicFontUrl = 'https://fonts.gstatic.com/s/notosansarabic/v18/nwpxtLGrOAZMl5nJ_wfgRg3DrWFZWsnVBJ_sS6tlqHHFlhQ5l3sQWIHPqzCfyG2vu3CBFQLaig.ttf';
              ...
              if (arabicResponse.ok) {
                ...
                console.error(`CDN Arabic font fetch failed: ${arabicResponse.status} ${arabicResponse.statusText}`);
              }
            } catch (err) {
          ...
          const englishLocalPaths = ['./Roboto-Regular.ttf', '/Roboto-Regular.ttf', 'Roboto-Regular.ttf'];
          ...
          if (!englishFontData && arabicFontData) {
            ...
            console.log(`✓ English font using Noto Sans Arabic fallback: ${englishFontData.byteLength} bytes`);
          }
          ...
          if (hasArabicCaptions) {
            if (!arabicFontData || arabicFontData.byteLength < 10000) {
              throw new Error(`Arabic font loading failed. Size: ${arabicFontData?.byteLength || 0} bytes`);
            }
          }
          
          // English font is always required (for English-only or mixed captions)
          if (!englishFontData || englishFontData.byteLength < 10000) {
            throw new Error(`English font loading failed. Size: ${englishFontData?.byteLength || 0} bytes. Last error: ${lastError?.message || 'Unknown'}`);
          }
          ...
          if (!arabicFontData || !englishFontData) {
            throw new Error(`Font data missing: arabic=${!!arabicFontData}, english=${!!englishFontData}`);
          }
          ...
          try {
            ...
            console.log(`✓ Font verification: arabic.ttf=${arabicSize} bytes, default.ttf=${englishSize} bytes`);
            
            if (arabicSize !== arabicFontData.byteLength || englishSize !== englishFontData.byteLength) {
              console.warn(`⚠ Font size mismatch - expected arabic=${arabicFontData.byteLength}, got=${arabicSize}, expected english=${englishFontData.byteLength}, got=${englishSize}`);
            }
          } catch (verifyError) {
          ...
          throw new Error(`FONT_LOAD_ERROR: Cannot export with captions - font loading failed. ${fontError instanceof Error ? fontError.message : String(fontError)}`);
        }
      }
      ...
      let args = buildFFmpegCommand(startTime, duration, captions, settings, 'input.mp4', 'output.mp4', canUseStreamCopy, inputWidth, inputHeight);
      ...
      if (!uint8Data && canUseStreamCopy) {
        ...
        args = buildFFmpegCommand(startTime, duration, captions, settings, 'input.mp4', 'output.mp4', false, inputWidth, inputHeight);
      ...
      if (!uint8Data && captions.length > 0) {
        ...
        args = buildFFmpegCommand(startTime, duration, [], settings, 'input.mp4', 'output.mp4', true, inputWidth, inputHeight);
        uint8Data = await executeAndCheck(args);
        ...
        if (!uint8Data) {
          ...
          args = buildFFmpegCommand(startTime, duration, [], settings, 'input.mp4', 'output.mp4', false, inputWidth, inputHeight);
          uint8Data = await executeAndCheck(args);
        }
      ...
      if (error instanceof Error) {
        ...
        // Video CORS errors
        if ((error.message.includes('CORS') || error.message.includes('Failed to fetch video')) && !error.message.includes('font')) {
